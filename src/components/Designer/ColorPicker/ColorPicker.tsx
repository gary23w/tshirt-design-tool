...
function ColorPicker({ getColor, defaultColor }: Props): ReactElement {
  const [color, setColor]: [any, (color: any) => void] = useState({
    R: 100,
    G: 100,
    B: 100,
    A: 100,
  ...
  const [colorGary, setColorGary] = useState("#aabbcc");
  ...
  const handleOnChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setColor(_color);
    getColor(
      `rgba(${to255(color.R)},${to255(color.G)},${to255(color.B)},${to255(
        color.A
      )})`
    );
  };

  const handleManualColorChoice = () => {
    console.log("color box");
  };

  console.log(setColorGary);
  return (
    <div className="d-flex flex-row align-items-center slider-colors">
      <Form className="slider-form-colors">
        <Form.Group controlId="color-slider">
          <div className="d-flex flex-column align-items-center">
            <Form.Label>Text Color</Form.Label>
            <HexColorPicker color={colorGary} onChange={setColorGary} />
            <div
              id="color-showcase"
              style={{
                borderWidth:
                  color.R < 100 || color.G < 100 || color.B < 100 ? 0 : "2px",
                backgroundColor: `rgba(${to255(color.R)},${to255(
                  color.G
                )},${to255(color.B)},${to255(color.A)})`,
              }}
              onClick={handleManualColorChoice}
            />
          </div>
